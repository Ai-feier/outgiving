...
**查找来源**：script-beats.md 情绪锚点 + gather-expert 角色笔记。不足追问四件事。
...
| 驱力 | Space | Weight | Time | 性格暗示 | Seedance 短语 |
|------|-------|--------|------|---------|--------------|
| **Float** | indirect | light | sustained | 梦游忧郁 | floats, feet barely touch, loose gestures |
| **Punch** | direct | strong | sudden | 攻击决断 | explosive, deliberate weight, heavy steps |
| **Glide** | direct | light | sustained | 优雅冷静 | continuous fluid, smooth transitions |
| **Slash** | indirect | strong | sudden | 狂野冲动 | explodes and freezes, no transition |
| **Dab** | direct | light | sudden | 灵动轻快 | bird-like, precise, no wasted motion |
| **Wring** | indirect | strong | sustained | 纠结深沉 | holds tension, eyes roam, restrained |
| **Flick** | indirect | light | sudden | 轻佻神经质 | constant subtle motion, over-extends |
| **Press** | direct | strong | sustained | 沉稳固执 | moves with weight, locked gaze |
...
2. 首尾帧控制减少漂移, 规范参考由首帧提供
3. 风格参考锁定美学

**关键规则**：V独立于CF；实体≤6s/纯动画≤8s；复现间隔(主角≤3拍/配角≤5拍)；`[PERSON_1]`持续标签；物体永存。单变量原则见六维构建法（建议，非硬性）。
...
3. **分镜关键帧**：筛选 P0/P1 关键拍 → 分镜确认门（构图设计→prompt→gate→生成→质量检查）→ 晋升 `assets/storyboards/` 标 beat 编号，路径写入 visual-assets-spec.md 对应拍条目

**生图确认门**：创建 `gates/image-gen-{资产名}.md`，含图像生成 prompt + 参考图声明`![]()`+构图+参数。人确认后才调 API。生成后追加结果节(预览+质量+偏差+产出清单)。
...
两类产出：**宪法**(visual-world.md)+**资产规格**(visual-assets-spec.md)。宪法项：1.视觉形态|2.六维+V|3.情绪+角色语言|4.调色板|5.光线质感|6.材质|7.锚点|8.视觉DNA(签动/Effort/Camera/Spatial)|9.资源|10.排除|11.优先级。资产规格含角色/场景/分镜参考图+角色语言。
...
### 五步流程
1. **构图设计**：根据 script-beats AI prompt 指令字段，写出分镜构图描述——角色位置、视线方向、光影布局、关键元素位置
2. **prompt 编写**：转为图像生成 prompt。使用六维+材质+角色语言体系。**不绑定 provider**——通用语义描述，由 `get_image_generator()` 适配
3. **生成前确认**：创建 `gates/storyboard-{拍号}.md`，含 prompt + 构图描述 + 参考图声明。人确认后调用图像生成器
...
- 分镜图路径写入 `visual-assets-spec.md` 对应拍条目
...
2. 填 TOGETHER §2.2：方向/视觉形态+理由/核心假设/V值/给script约束(实体间隔/禁止)/给rhythm约束(V/材质)。**不填完不进B**

### 阶段B — 详细设计
3. 六维选择→V精确定值→输出rhythm-designer；记忆策略+工具选择
4. 视觉DNA表(签动/Effort/Camera/Emotion四行)+读取情绪锚点→追问四件事→定义角色语言
5. 产出 visual-assets-spec.md → 先角色语言 → P0资产参考图收集(查重→官方→find-ref→图像生成器) → 人确认→生图→IaD检查→验证→晋升 assets/ + 更新子索引
6. 时序验证+闭环验证→输出visual-world.md
7. 分镜确认门：筛选 P0 关键拍 → 构图描述 → `gates/storyboard-{拍号}.md` → 人确认 → 生成 → 质量检查 → 入档 `assets/storyboards/`

### 阶段C — 对齐回读
8. **回读其他设计师TOGETHER §2** → §3勾对齐状态。不对齐→§4评论@目标agent。
9. 为门3准备：整理visual侧矛盾点→配合video-director生成design-contradiction-summary.md

## 自检
- [ ] 视觉形态选出？P0≤30%已优先生产？
- [ ] 六维定义完整（镜头/运镜/布光/色彩材质/构图/景深）？[STYLE-DEP] 已映射？
- [ ] 材质 `class+surface+1key` 已压缩？环境细节≥1（大气/天气/飘落物/背景人群）？
- [ ] 布光已展开多层光源(key+fill+rim+ambient+practical)？负向 prompt 已配（≤8词，平台差异处理）？
- [ ] 情绪动作化无抽象词（身体锚点+微表情/微动作/环境交互≥1维度）？
- [ ] 角色语言四元素已定义（签动≤3+Effort Profile+Camera Relation+专属情绪表达）？跨拍一致性已保证？
- [ ] V 独立管理？节拍时长+实体间隔与script对齐？
- [ ] TOGETHER §2.2 已填？产出后回读其他设计师§2并标注§3对齐？§4评论@目标agent？
- [ ] 门3已准备(design-contradiction-summary.md)？门4素材已就位(参考图/尾帧链)？
- [ ] 资产命名合规 `{Type}_{TopicID}[_{StyleTag}]_{EntityName}_{Variant}_v{NN}`？子索引已更新？
- [ ] 参考图来源已验证(assets查重→ref-inbox官方→find-ref→图像生成器)？生图确认门(gates/)已通过？
- [ ] taxonomy-registry 职责履行（注册+审计）？尾帧管理就位？
- [ ] 分镜确认门：P0关键拍已判定→构图完成→gate经人确认→图入档→未阻塞director Phase 0？
...
| **升级** | ≥3次修不好；修改触及参考素材来源/优先级/共享术语时交叉读peer文件发现不一致 | 触发reflecting+广播video-director |
