...
| CF<2 单边约束 / V 独立管理 | rhythm↔visual | director |
...
| **ai-video 尾帧传递协议**(非首段必用前段末帧；命名`{project}-{segment-id}-lastframe.png`；存`ai-video/projects/TXXX/assets/last-frames/`；场景切换需定调图替代) | director | visual-designer(末帧提取写入) |
...
6. **生图确认门预检**：读 §4 + `material-backlog-TXXX.md` + `gates/image-gen-*`，确认 P0 资产状态：
   - 已就绪（`ref-images/` 有 .png + IaD 已验证）→ 继续
   - 未就绪但门已过（`gates/image-gen-*.md` 存在）→ 标注「P0 生产中」，IaD/RefImg 标记 PENDING
   - 未就绪且门未通过 → 阻塞。写入 `material-backlog-TXXX.md`
   - **检查顺序**：asset-lab → `ref-images/` → `gates/image-gen-*`。不跳过
...
2. 第 N 段（N>1）：N-1 段末帧作 Tier 0 参考图嵌入 prompt。生成后同样提取末帧 → `last-frames/{project}-BN-lastframe.png`
...
### 反馈循环（生成后）
生成视频回检：(1)锚点拍传达叙事功能？(2)视觉一致性保持？(3)音频-视觉边界对齐？不一致→查源：prompt→用 `uv run --directory scripts ai generate video` 重生成；工具→容错/换；上下文→拆段/降密度。
...
**前置**：读 script-beats.md 视觉资产需求清单 → 对账 asset-lab 库存得出缺口。实体规范由首拍锁定。

---

**Step 1 — 库存清单**

以需求清单索引 `asset-lab.md`，按 `appears_in_style` 过滤后名称匹配。100+ 行时启用 `_scripts/asset-query.sh`。

- 命中 → 记录名称/风格/版本/状态/质量/路径
- 未命中 → 查 `ref-images/` 物理文件（草稿→「生产中」）；非首段查 `last-frames/`（末帧强制配发，非缺货）
...
**2a. 质量筛选**（原 Step 2-3 合并）：
| 条件 | 裁决 |
|------|------|
| `active` + `✓` | 直接引用，选最高版本 |
| `active` + `⚠` | 可用。P0→记录风险+prompt 标注一致性注意点；P1→正常引用 |
| `active` + `✗` | 不可用。P0→阻塞合成并退回；P1→降级为文本描述 |
| `deprecated` | 同一名无 `active` 版本时临时引用（标注"已过时，待重产"） |
| `archived` | 视为不存在，走缺货 |

退回 visual-designer 格式：`退回：资产名 | 缺陷 | 优先级`

**2b. 缺货处理**：
检查顺序：`asset-lab` 无 → `ref-images/` 有物理文件？→ `gates/image-gen-*` 通过？

| 状态 | 处理 |
|------|------|
| P0 + 生图确认门未通过 | 阻塞合成。写入 `material-backlog-TXXX.md` |
| P0 + 门已过 / `ref-images/` 有草稿 | 部分阻塞。IaD/RefImg 标记 PENDING，合成可继续 |
| P1 | 不阻塞。降级为文本描述，prompt 标注 `[素材缺失]` |

退回文档可读表格：资产名 | 缺陷 | 优先级 | 次数 | 阻塞拍 | 状态。`material-backlog-TXXX.md` 清零为合成前置条件。

**2c. 候选池裁剪**（超硬上限时启用）：

| 优先级 | 条件 |
|--------|------|
| Tier 0 | 非首段强制末帧（不计入 9 图槽） |
| Tier 1 | 同风格+IaD✓+同选题+最新 |
| Tier 2 | 同风格+IaD✓+同选题+较早 |
| Tier 3 | 同风格+IaD✓+跨选题复用 |
| Tier 4 | 同风格+IaD⚠+无更优替代 |
| Tier 5 | 跨风格(标注风险) / deprecated无active替代 |

同 Tier 选视角/光照互补填 ≤9 图槽。视频/音频 ≤3，选运镜多样。`appears_in_style` 不含当前风格→降权至 Tier 5。前置门禁：总候选图=0→阻塞 | 含角色视频缺角色参考图→P0阻塞 | 非首段缺前段末帧且非场景切换→阻塞。`material-backlog-TXXX.md` 清零为合成前置条件。

---

**Step 3 — 嵌入 prompt（参考图声明 + 字段嵌入 + 完整性验证）**

**3a. 参考图声明块**（每段 prompt 头部）：
```
- ![描述](路径) [用途标签] [来源标注]
- [负面参考] ![描述](路径) [原因，不占用 9 图槽]
```

格式：`![]()` 内联 + 用途标签 + 来源链。负面 `[负面参考]` 前缀。末帧标注 `[段间锚定:N→N+1]` 不绑角色。角色图绑定实体 `<tag>`。

**3b. 嵌入 prompt 字段**：
图片 `<图片N>` / 视频 `<视频N>` / 音频 `<音频N>` 嵌入 prompt 对应字段。角色图用 `@` 分配语法绑定标签。末帧标注：`<图片N> [段间锚定:N→N+1] @ImageN as last-frame transition reference (no 角色绑定——纯外观延续)`。非首段 subject 字段开头嵌入 `，承接前段末帧` 语义锚定。

**3c. 完整性验证**（装配完成后，合成开始前）：
- [ ] 需求对账：P0→已收集/缺货阻塞/降级P1
- [ ] 质量筛选：无 `✗` 入 prompt
- [ ] 总量 ≤9图+≤3视频+≤3音频，关键帧≤4/段
- [ ] 角色图 IaD 中性（≥3 张时每张 < 5s 目测）
- [ ] 逐拍检查 `@` 绑定覆盖该拍出现实体
- [ ] 每段 ≥1 参考图（首段→asset-lab/ref-images；非首段含末帧或场景定调图）
- [ ] 非首段末帧路径存在可访问
- [ ] `material-backlog-TXXX.md` 已清零
- [ ] 声明块路径与 `@` 分配一致
...
**光照-情绪映射速查**（情绪外化替代——不写"悲伤"，写光照条件）：
| 情绪 | 光照条件 |
|------|---------|
| 希望/温暖 | golden hour backlight + warm amber |
| 绝望/压抑 | low-key + hard overhead + deep shadows |
...
硬切0s / 交叉溶解0.5-1.5s / 渐黑白1-2s / 匹配剪辑0s / 音频先行0.3-1s / 快速横摇(whip pan 0.3-0.5s)。

`[STYLE-DEP]` japanese-anime 禁止 whip pan / fast spin；首选交叉溶解+硬切。动作低缓连续是跨风格基础原则——快速运镜增加 motion blur 和身份漂移风险。
...
单段上限：Seedance 2.0 15s(mini/fast)/30s(pro)† / Kling 3.0 15s / Veo 60s / Hailuo 20s+ / Runway 18s。Seedance 分辨率480p/720p(arXiv:2604.14148)，Pro上限更高但适配器硬限15s。†CLI 硬限15s，30s 需 CLI 分支。超上限→拆段或换适配器。
...
| **统一调用入口** | `uv run --directory scripts ai generate video`（自动读 video-prompt.md） | `ai generate image` / `ai extract-lastframe` |
...
- [ ] **TOGETHER.md 对齐**：§3 无🔴；§4 评论已核实并入矛盾矩阵
...
| **制作**(条件)：video-prompt.md(工具语法+实体注册表+SDK检验+收敛门+生成调度+关键帧) | 设计确认 | `uv run --directory scripts ai generate video` / 生成操作员 |

video-prompt.md 两层：**叙事段**（人读，12要素 Shot-by-Shot 表格 — 设计评审）→ **格式化段**（机器读，9要素命名字段+全局前缀 — 直接输入 CLI）。
...
遵循reflecting漏斗模型。入口：裁决被纠正≥2次/合成不对/语法breaking/品味否决被滥用/观察驱动(工具/模型/流程堵点，同一现象≥2次)。

| 深度 | 触发 | 做法 |
|------|------|------|
| 操作层 | 同类纠正≥2次/自检被绕过 | 最小修改.md |
| 设计层 | 深度1无效/与agent冲突/共享工具名/命令/版本号未同步collaboration | 重构规则组→memory |
