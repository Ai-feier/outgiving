...
每次用后追加 `refinement-log.md`。格式：`精进 #{N} — {日期} | 需求：{声明} | 命中：Y/N | 有效策略：{...} | 无效策略：{...} | 优化：{...}`
触发：连续 2 次 P0-P4 全未命中 / 来源失效 / 发现新站点。
**原则（减法优先）**：已有策略完整执行后无效才有理由新增。
...
不下载有「禁止转载/商用」声明的素材；不直接用于发布（仅 AI 生成参考）；不复制到平台发布目录。

## 质量门禁

□ 五步全部走完 □ 素材规范存入 ref-images/ □ ref-sources.md 已生成
□ 通过质量+版权检查 □ gate 对接段落已准备 □ 搜索日志+精进日志已更新
