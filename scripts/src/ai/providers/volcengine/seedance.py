...
from ai.json_types import ArkEnvelope
...
class SeedanceVideo:
    ...
    def build_request(self, prompt: VideoPrompt) -> dict[str, Any]:
        """预览 ARK 请求体（CLI --dry-run 用）。"""
        return self._build_request(prompt)
    ...
    def _parse_response(self, data: ArkEnvelope, task_id: str) -> VideoResult:
        """解析 ARK API 响应。"""
        status_str = str(data.get("status") or "")
        ...
        if err:
            ...
            error_msg = raw_msg if isinstance(raw_msg, str) else None
